...
class NuScenesTraj(object):
    ...
    def get_history_traj_label(self, sample_token):
        ...
        history_traj = []
        for _ in range(self.planning_steps):
            prev_annotation_token = sample['prev']
            if prev_annotation_token == "":
                break
            sample = self.nusc.get('sample', prev_annotation_token)
            sd_rec = self.nusc.get('sample_data', sample['data']['LIDAR_TOP'])
            rec_pose_record = self.nusc.get('ego_pose', sd_rec['ego_pose_token'])
            rec_e2g_rot = Quaternion(rec_pose_record['rotation']).rotation_matrix
            rec_e2g_trans = np.array(rec_pose_record['translation'])
            prev_bbox3d = self.generate_sdc_info(as_lidar_instance3d_box=True)
            
            prev_bbox3d.rotate(rec_e2g_rot.T)
            prev_bbox3d.translate(rec_e2g_trans)
            
            prev_bbox3d.translate(-ref_e2g_trans)
            prev_bbox3d.rotate(ref_e2g_rot)
            history_traj.append(prev_bbox3d)

        history_traj_all = np.zeros((1, self.planning_steps, 3))
        history_traj_mask_all = np.zeros((1, self.planning_steps, 2))
        n_valid_timestep = len(history_traj)
        if n_valid_timestep > 0:
            history_traj = [p.tensor.squeeze(0) for p in history_traj]
            history_traj = np.stack(history_traj, axis=0)  # (valid_t, 9)
            history_traj = history_traj[:, [0,1,6]]  # (x, y, yaw)
            history_traj_all[:,:n_valid_timestep,:] = history_traj
            history_traj_mask_all[:,:n_valid_timestep,:] = 1
        
        return history_traj_all, history_traj_mask_all
    ...
    def get_past_traj_label(self, sample_token):
        ...
        num_box = len(annotations)
        gt_prev_trajs = np.zeros((num_box, self.prediction_steps, 2))
        gt_prev_yaw = np.zeros((num_box, self.prediction_steps))
        gt_prev_masks = np.zeros((num_box, self.prediction_steps))
        gt_prev_idx = np.zeros((num_box, 1))
        for i, anno in enumerate(annotations):
            gt_prev_idx[i] = self.cat2idx[anno['category_name']] if anno['category_name'] in self.cat2idx.keys() else -1
            cur_box = boxes[i]
            cur_box.rotate(Quaternion(cs_record['rotation']))
            cur_box.translate(np.array(cs_record['translation']))
            cur_anno = anno
            for j in range(self.prediction_steps):
                if cur_anno['prev'] != '':
                    anno_prev = self.nusc.get('sample_annotation', cur_anno['prev'])
                    box_prev = Box(
                        anno_prev['translation'], anno_prev['size'], Quaternion(anno_prev['rotation'])
                    )
                    # Move box to ego vehicle coord system.
                    box_prev.translate(-np.array(pose_record['translation']))
                    box_prev.rotate(Quaternion(pose_record['rotation']).inverse)

                    gt_prev_trajs[i, j] = box_prev.center[:2] - cur_box.center[:2]
                    gt_prev_masks[i, j] = 1
                    # add yaw diff
                    box_yaw = cur_box.orientation.yaw_pitch_roll[0]
                    box_yaw_pred = box_prev.orientation.yaw_pitch_roll[0]
                    gt_prev_yaw[i, j] = box_yaw_pred - box_yaw
                    cur_anno = anno_prev
                    cur_box = box_prev
                else:
                    gt_prev_trajs[i, j:] = 0
                    break
        return gt_prev_trajs, gt_prev_yaw, gt_prev_masks, gt_prev_idx
