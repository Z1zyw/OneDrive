...
def main(args):
    ...
    for data in key_infos['infos']:
        ...
        if os.path.exists(os.path.join(pred_path, data['token'])):
            with open(os.path.join(pred_path, data['token']),'r',encoding='utf8')as f:
                ...
                if full_match:
                    ...
                    preds[data['token']] = np.array(coordinates)
