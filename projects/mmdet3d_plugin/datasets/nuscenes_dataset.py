...
class CustomNuScenesDataset(NuScenesDataset):
    ...
    def __init__(self, 
                 ...
                 eval_mode=['lane', 'det'],
                 single_scene_debug=False,
                 single_sample_debug=False,
                 *args, 
                 **kwargs):
        ...
        if single_scene_debug:
            self.data_infos = self.data_infos[:40] # first scene
        if single_sample_debug:
            self.data_infos = self.data_infos[:1] # first sample
    ...
    def get_data_info(self, index):
        ...
        if 'gt_history_traj' in info.keys():
            input_dict['gt_history_traj'] = info['gt_history_traj']
        if 'gt_history_traj_mask' in info.keys():
            input_dict['gt_history_traj_mask'] = info['gt_history_traj_mask']
    ...
    def evaluate(self,
        ...
        """
        results_dict = dict()
        # hack save for visualization
        # HACK_SAVE=False 
        # if HACK_SAVE:
        #     import pickle # for debug
        #     pickle.dump(results, open('v17_debug_evaluate_results_v17_Det_Map_Text_rm_temp_E2E_7665.pkl', 'wb')) # tmp for debug
        
        if 'lane' in self.eval_mode and len(results[0]['lane_results']) > 0:
            predictions = {
                'method': 'dummy',
                'authors': ['dummy'],
                'e-mail': 'dummy',
            ...
            for index, result in enumerate(results):
                prediction = {                
                    ...
                    'topology_lclc': np.zeros((600, 600)),
                    'topology_lcte': np.zeros((600, 0)),
                }
            ...
            metric_results = {}
            # TODO, save Lane prediction
            HACK_SAVE=True
            if HACK_SAVE:
                import pickle
                pickle.dump(predictions, open('in_v17_onlyE2E_Det_Mapv2_Text_rm_temp_32x16_wAll_lr2_fixedPos_fixHeadDim_v10head_sota_version.pkl', 'wb'))
            # 太慢了
