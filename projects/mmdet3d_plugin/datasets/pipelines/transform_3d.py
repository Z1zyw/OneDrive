    ...
    def __init__(self,
        ...
        if img_scale is None:
            ...
            if isinstance(img_scale, list):
                ...
                self.img_scale = [img_scale]
            assert mmcv.is_list_of(self.img_scale, tuple)  # 使用 mmengine 的 is_list_of
        ...
        self.use_constructive = use_constructive
...
def format_bin_num(n):
    return '<<' + str(n) + '>>'

def format_action_steer(n, yaw=5):
    if yaw==5:
        steering_options = ["right2", "right1", "straight", "left1", "left2"]
    elif yaw==7:
        steering_options = ["right4", "right3", "right2", "right1", "straight", "left1", "left2"]
    if 0 <= n < len(steering_options):
        return steering_options[n]
    else:
        raise ValueError(f"Invalid steering option: {n}")

def format_action_accel(n,accl=7):
    if accl==7:
        steering_options = ["slow2", "slow1", "maintain", "fast1", "fast2", "fast3", "fast4"]
    elif accl==9:
        steering_options = ["slow3", "slow2", "slow1", "maintain", "fast1", "fast2", "fast3", "fast4", "fast5"]
    if 0 <= n < len(steering_options):
        return steering_options[n]
    else:
        raise ValueError(f"Invalid steering option: {n}")
...
class LoadAnnoatationVQA():
    def __init__(
            ...
            img_len=400,
            img_num=6,
            post_query=False, # flag for whether to place query token after image tokens
            use_qwen_vl=False,
            lane_objs_info=None):
        ...
        if 'llava' in tokenizer:
            assert not use_qwen_vl, "LLaVA tokenizer is not compatible with Qwen-VL."
    ...
    def __call__(self, results):
        ...
        results['vlm_text'] = vqa_anno # For DEBUG

        vqa_converted = preprocess([vqa_anno], self.tokenizer, True, 
                                   use_qwenvl_25=self.use_qwen_vl, 
                                   query_len=self.query_len, 
                                   img_num=self.img_num,
                                   img_len=self.img_len, post_query=self.post_query)
...
class LoadAnnoatationVQATest():
    def __init__(
            ...
            use_qwen_vl=False,
            post_query=False,  # flag for whether place query token after image tokens
            query_len=600,
            img_len=400,
            img_num=6,
            ):
        """
        Initialize VQA test annotation loader
        
        Args:
            base_conv_path: Path to conversation data
            base_vqa_path: Path to VQA data
            tokenizer: Tokenizer for text processing
            max_length: Maximum sequence length
            base_counter_path: Path to counter data
            load_type: Types of data to load
        ...
        self.use_pred_traj_seq = False
        
        # Template questions for conversation
        self.template = [
                        "What can you tell about the current driving conditions from the images?",
                        "What can be observed in the panoramic images provided?",
                        "Can you provide a summary of the current driving scenario based on the input images?",
    ...
    def trans_json_to_traj(self, traj_path):
        """Convert JSON trajectory to numpy array"""
    ...
    def __call__(self, results):
        ...
        has_traj = self.use_pred_traj_seq
        vqa_converted = preprocess(sources, self.tokenizer, True, False, has_traj=has_traj, use_qwenvl_25=self.use_qwen_vl, query_len=self.query_len, img_len=self.img_len, post_query=self.post_query, img_num=self.img_num)
...
class ResizeCropFlipRotImage():
    def __init__(self, data_aug_conf=None, with_2d=True, filter_invisible=True, training=True, use_constructive=False):
