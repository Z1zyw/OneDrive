...
class UniDETHeadSeq(AnchorFreeHead):
    ...
    def __init__(self,
        ...
        if class_weight is not None and (self.__class__ is UniDETHeadSeq):
        ...
        self.use_commandemb = use_commandemb
        ...
        self.add_can_bus_embed = add_can_bus_embed
        self.scalar = scalar # dn training hyperparameter
        self.bbox_noise_scale = noise_scale
        self.bbox_noise_trans = noise_trans
        self.dn_weight = dn_weight
        self.split = split 
        self.set_yaw_zero = set_yaw_zero
        self.input_norm = input_norm
        # self.act_cfg = transformer.get('act_cfg',
                                    #    dict(type='ReLU', inplace=True))
        self.num_pred = num_pred
        self.normedlinear = normedlinear
        super(UniDETHeadSeq, self).__init__(num_classes, in_channels, init_cfg = init_cfg)
    ...
    def _init_layers(self):
        ...
        self.reference_points = nn.Embedding(self.num_query, 3) # (num_query, 3)
        if self.use_learnable_query:
            self.obj_learnable_query = nn.Embedding(self.num_query, self.embed_dims)
        ...
        # if self.can_bus_len == 5:   
        #     input_len = 116
        # elif self.can_bus_len == 4:
        #     input_len = 102
        # elif self.can_bus_len == 3:
        #     input_len = 88
        # elif self.can_bus_len == 2: 
        #     input_len = 74
        # elif self.use_commandemb:
        #     input_len = 71

        # self.can_bus_embed = nn.Sequential(
        #     nn.Linear(input_len, self.embed_dims*4), # canbus + command + egopose
        #     nn.ReLU(),
        #     nn.Linear(self.embed_dims*4, 4096),
        # )
    ...
    def init_weights(self):
        ...
        # # 原始代码貌似没有对 reg head 的初始化
        # for reg_branch in self.reg_branches:
        #     for m in reg_branch.modules():
    ...
    def post_update_memory(self, data, rec_ego_pose, all_cls_scores, all_bbox_preds, outs_dec, mask_dict, rec_can_bus):
    ...
    def _load_from_state_dict(self, state_dict, prefix, local_metadata, strict,
        ...
        if (version is None or version < 2) and self.__class__ is UniDETHeadSeq:
            convert_dict = {
    ...
    def get_query_embedding(self, batch_size:int, data):
        ...
        query_pos = self.query_pos(nerf_positional_encoding(reference_points.repeat(batch_size, 1, self.n_control)))
        reference_points = reference_points.repeat(batch_size, 1, 1)

        query_embed = torch.zeros_like(query_pos)
        if self.use_learnable_query:
            query_embed = self.obj_learnable_query.weight.unsqueeze(0).repeat(batch_size, 1, 1)
        
        query_embed, query_pos, reference_points, temp_memory, temp_pos, rec_ego_pose = self.temporal_alignment(query_pos, query_embed, reference_points)

        # Maybe BUG here: self.rec_ego_pose
        self.rec_ego_pose = rec_ego_pose
        
        # query_embed = torch.cat([temp_memory, query_embed], dim=1)
        # query_pos = torch.cat([temp_pos, query_pos], dim=1)
        query_embed = torch.cat([query_embed, temp_memory], dim=1)
        query_pos = torch.cat([query_pos, temp_pos], dim=1)
        
        return query_embed, query_pos, reference_points
    ...
    def forward(
        ...
        **kwargs):
        # B = query.shape[0]
        # reference_points = self.reference_points.weight
        # reference_points = torch.cat([img_ref[0], reference_points], dim=0)
        ...
        # # TODO, move can_bus_input implementation to UniVLA
        # if self.set_yaw_zero:            
        #     data['can_bus'][:,9]= 0
        # if self.use_commandemb:
        #     rec_can_bus = data['can_bus']
        # else:
        ...
        # can_bus_input = torch.cat([rec_can_bus, self.memory_canbus.flatten(-2), memory_ego_pose.mean(-2).flatten(-2)], dim=-1)
        # # can_bus_embed = self.can_bus_embed(can_bus_input).unsqueeze(-2)
        ...
        if mask_dict and mask_dict['pad_size'] > 0:
            ...
            outs = {
                ...
                # # 'can_bus_embed': can_bus_embed.unsqueeze(-2) if self.add_can_bus_embed else None,
                # 'can_bus_embed': None,
                # # 'rec_can_bus': can_bus_input,
            }
        else:
            outs = {
        ...
        _ = self.post_update_memory(data, self.rec_ego_pose, all_cls_scores, all_bbox_preds, outs_dec, mask_dict=None, rec_can_bus=None)
        return outs
