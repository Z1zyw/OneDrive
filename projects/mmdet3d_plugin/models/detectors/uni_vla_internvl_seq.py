...
from ..utils.misc import load_uni_model
...
HACK_INFERENCE_BENCHMARK=False
...
class UniVLAInternVLSeq(MVXTwoStageDetector):
    """Petr3D."""
    def __init__(self,
                 ...
                 use_same_query=True):
        super(UniVLAInternVLSeq, self).__init__(pts_voxel_layer, pts_voxel_encoder,
                             pts_middle_encoder, pts_fusion_layer,
                             img_backbone, pts_backbone, img_neck, pts_neck,
                             pts_bbox_head, img_roi_head, img_rpn_head,
                             train_cfg, test_cfg, pretrained)
        self.use_internvl_visual = use_internvl_visual 
        ...
        if not use_internvl_visual and frozen_vit:
        ...
        if lm_head is not None:
            ...
            for name, param in self.lm_head.named_parameters():
                ...
                if 'mlp1' in name or 'mlp2' in name: # InternVL vision-llm alignment mlp
        ...
        if not text_finetune:
            if use_lora:
                self.lm_head.base_model.model.language_model.model.layers = self.lm_head.base_model.model.language_model.model.layers[:6]
                self.lm_head.base_model.model.language_model.model.norm = nn.Identity()
                
                self.lm_head.base_model.model.language_model.lm_head = None
    ...
    def prepare_location(self, img_metas, out_split_location=False, **data):
        ...
        if out_split_location:
            bn, h, w,  _ = location.shape
            location_l = location[..., :w//2, :].flatten(1,2)
            location_r = location[..., w//2:, :].flatten(1,2)
            location = torch.cat([location_l, location_r], dim=1).reshape(bn, h, w, -1)
        return location
    ...
    def get_img_feats_by_Internvl_vision_model(self, img):
        # img.shape = [b, n, c, h, w]
        assert self.lm_head.vision_model is not None
        batch_size, num_camera, channels = img.shape[:3]
        img = img.reshape(-1, *img.shape[2:])  # [b*n, c, h, w]
        if self.use_grid_mask:
            img = self.grid_mask(img)
        img = img.to(self.lm_head.vision_model.dtype)
        if self.split_img:
            if img.shape[-1]!=896:
                raise NotImplementedError
            img_l = img[..., :448]
        ...
        img_embeddings = self.lm_head.extract_feature(pixel_values=img)
        if self.split_img:
            emb_l = img_embeddings[:batch_size*num_camera]
            emb_r = img_embeddings[batch_size*num_camera:]
            img_embeddings = torch.cat([emb_l, emb_r], dim=1)

        return img_embeddings
    
    def get_img_feats_by_Internvl_vision_model_with_thumbnail(self, img):
        # img.shape = [b, n, c, h, w]
        assert self.lm_head.vision_model is not None
        batch_size, num_camera, channels = img.shape[:3]
        img = img.reshape(-1, *img.shape[2:])  # [b*n, c, h, w]
        if self.use_grid_mask:
            img = self.grid_mask(img)
        img = img.to(self.lm_head.vision_model.dtype)
        img_thumbnail = torch.nn.functional.interpolate(img, size=(448,448), mode='bilinear', align_corners=False)
        if self.split_img:
            if img.shape[-1]!=896:
                raise NotImplementedError
            img_l = img[..., :448]
        ...
        img_input = torch.cat([img, img_thumbnail], dim=0)
        img_embeddings_all = self.lm_head.extract_feature(pixel_values=img_input)
        img_embeddings, img_thumbnail_embeddings = torch.split(img_embeddings_all, img.shape[0], dim=0)
        
        if self.split_img:
            emb_l = img_embeddings[:batch_size*num_camera]
            emb_r = img_embeddings[batch_size*num_camera:]
            img_embeddings = torch.cat([emb_l, emb_r], dim=1)
        return img_embeddings, img_thumbnail_embeddings
    ...
    def forward_pts_train(self,
        ...
        if not self.use_thumbnail:
            if self.use_internvl_visual:
                img_embedding = self.get_img_feats_by_Internvl_vision_model(data['img'])
                # hack
                h, w = data['img'].shape[-2] // 28, data['img'].shape[-1] // 28
                data['img_feats'] = img_embedding.view(B, N_CAM, h, w, self.embed_dims).permute(0, 1, 4, 2, 3).contiguous() 
                img_embedding = img_embedding.view(B, -1, self.embed_dims)
            else:
                # using EVA backbone
                # # hack imple
                # t, h, w = 1, 40, 40 # hack
            ...
            location = self.prepare_location(img_metas, out_split_location=self.split_img, **data)
            pos_embeds_3d = self.position_embeding(data, location, img_metas)
        else:
            assert self.use_internvl_visual, "thumbnail only support internvl visual model"
            img_embedding, img_thumbnail_embedding = self.get_img_feats_by_Internvl_vision_model_with_thumbnail(data['img'])
            h, w = data['img'].shape[-2] // 28, data['img'].shape[-1] // 28
            data['img_feats'] = img_embedding.view(B, N_CAM, h, w, self.embed_dims).permute(0, 1, 4, 2, 3).contiguous() 
            location = self.prepare_location(img_metas, out_split_location=True, **data)
            
            location_all = self.prepare_location(img_metas, **data)
            location_thumbnail = nn.functional.interpolate(location_all.permute(0,3,1,2), size=(16,16), mode='bilinear', align_corners=False).permute(0,2,3,1)
                
            pos_embeds_3d = self.position_embeding(data, location, img_metas)
            pos_embeds_3d_thumbnail = self.position_embeding(data, location_thumbnail, img_metas)
            
            img_embedding = torch.cat([img_embedding, img_thumbnail_embedding], dim=1)
            img_embedding = img_embedding.view(B, -1, self.embed_dims)
            
            pos_embeds_3d = pos_embeds_3d.view(B*N_CAM, -1, self.embed_dims)
            pos_embeds_3d_thumbnail = pos_embeds_3d_thumbnail.view(B*N_CAM, -1, self.embed_dims)
            pos_embeds_3d = torch.cat([pos_embeds_3d, pos_embeds_3d_thumbnail], dim=1)
            pos_embeds_3d = pos_embeds_3d.view(B, -1, self.embed_dims)
        ...
        if self.with_map_head:
            if not self.map_first:
                ...
                used_heads = ['map_head'] + used_heads
    ...
    def forward_train(self,
        ...
        if self.tokenizer is not None:
            ...
            vlm_labels = None
            vlm_attn_mask = None
        if not self.use_internvl_visual:
    ...
    def simple_test_pts(self, img_metas, **data):
        ...
        if len(used_heads) > 0:
            if 'task_hidden_states' in vlm_out:
                ...
                bidirectional_mask = data['input_ids'][0][0] == 151655 # HACK
    ...
    def simple_test_pts_e2e_only(self, img_metas, **data):
        """Test function of point cloud branch."""
        timing = {}
        torch.cuda.synchronize()
        total_start = time.time()
        ...
        if len(used_heads) > 0:
            ...
            for head_name in used_heads:
                if head_name != 'e2e_head': # hack implementation, only evaluate perception heads
                    continue
    ...
    def simple_test(self, img_metas, **data):
        ...
        if not HACK_INFERENCE_BENCHMARK:
            bbox_pts, generated_text, lane_results = self.simple_test_pts(
                img_metas, **data)
        else:
            self.simple_test_pts_e2e_only(
                img_metas, **data)
            # bbox_pts, generated_text, lane_results = self.simple_test_pts(
            #     img_metas, **data)
