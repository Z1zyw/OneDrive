...
def main():
    ...
    if args.gpu_ids is not None:
        ...
        cfg.gpu_ids = [torch.device('cuda', i) for i in range(torch.cuda.device_count())]
    ...
    model.init_weights() # 这个地方会重新 初始化 模型
    # BUG 包括预训练模型的权重（替换的模块 Fp16Qwen2_5_VLVisionFlashAttention2）都被重新初始化了
    # import ipdb; ipdb.set_trace()
    # # hack imple
    # for idx in range(6):
    #     model.lm_head.base_model.model.model.layers[idx].self_attn.task_adapter._init_weights()
    # # hack end
    
    
    # how to fix
    # try: # hack: Skip BUG of logging: handler.stream is None, cannot write log
    #     model.init_weights() # 
    # except:
    #     pass 
    ...
    custom_train_model(
        ...
        timestamp=timestamp,
        meta=meta)
    if cfg.model.get('finetune_emb',False):
        model.tokenizer.save_pretrained('/nfs/dataset-ofs-voyager-research/xschen/repos/OmniDrive/ckpts/pretrain_dist_token')
